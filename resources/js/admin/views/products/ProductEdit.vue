<template>
    <div class="p-5 bg-white rounded" v-if="product">
        <div class="mb-5 flex items-center">
            ...
            <router-link class="ml-auto bg-sky-600 text-white py-2 px-4 rounded transition-all hover:bg-sky-500" :to="{ name: 'products.show', params: { slug: product.slug } }">View</router-link>
        </div>
        <h1 class="font-bold text-3xl mb-3">Edit Product #{{ product.id }}</h1>
        <div v-if="response.message" class="mb-5 text-center">
            ...
            <div v-if="response.status == 422" v-for="(error, index) in response.message.errors" :key="index" class="text-red-500">{{ error[0] }}</div>
            <div v-else-if="response.status == 500" class="text-red-500">{{ response.message.error }}</div>
        </div>
        <div>
            <ProductForm :product="product" @xsubmit="xsubmit" />
        </div>
    </div>
</template>
